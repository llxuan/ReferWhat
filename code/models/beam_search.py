...
class Generator(object):
    ...
    def beam_search(self, rnn_input, visual_feature=None, initial_state=None):
        ...
        if visual_feature is not None :
            rnn_input = torch.cat([visual_feature, rnn_input],2)
        ...
        for _ in range(self.max_caption_length - 1):
            ...
            if visual_feature is not None:
                # concatenate visual feature on each input of RNN
                rnn_input = torch.cat([visual_feature.repeat(len(input_feed), 1, 1),embeddings],2)
            else:
                rnn_input = embeddings
            words, logprobs, new_states = get_topk_words(rnn_input, state_feed)
            for i, partial_caption in enumerate(partial_captions_list):
                if isinstance(new_states, tuple):
                    state = (new_states[0].narrow(1, i, 1),
                             new_states[1].narrow(1, i, 1))
                else:
                    state = new_states[:, i].unsqueeze(1)
                for k in range(self.beam_size):
                    w = words[i, k]
        ...
        return [c.sentence for c in caps], [c.score for c in caps], [c.state for c in caps]
